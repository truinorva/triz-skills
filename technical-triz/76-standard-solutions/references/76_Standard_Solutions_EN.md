...
| Type of interaction | Drawn as |
|---|---|
| Desired, useful effect | solid arrow |
| Insufficient effect | dotted arrow |
| Excessive effect | double arrow — two parallel lines, one arrowhead |
| Harmful effect | wavy arrow, drawn in red |
| Non-specific, general effect or interaction (e.g. field on substance) | plain line without arrowhead |
...
The scheme has six columns. Filled in for the worked example in the image:

1. **Problem** — description
2. **Interacting components** — Substance 1, Substance 2, Field 1
3. **S/F model** — the field acts on both substances, the effect between them is harmful:
...
4. **Possible standard solutions** — introduction of a new substance
5. **Chosen standard solution (new SFM)** — the new substance S3 modifies the interaction:
...
Notation used here: `~~` marks a harmful effect, `-|-` the modified interaction of the new SFM.
...
**Question:** Why does the system not work or only work insufficiently? → Is one of the substances or the interaction field not present?
...
- **When to use?** When no internal additives (Standard 1.1.2) and no external substances (Standard 1.1.3) are permitted or practicable→ When the environment already contains useful substances that have so far remained unused
...
- **When to use?**→ If neither the direct introduction of an internal additive (1.1.2) nor an external additive (1.1.3) is possible or effective and the existing system environment does not offer a directly available, usable substance.
...
- **Measure:** Create maximum effect through excess substance or field
  1. excess field is   newtralized by additional substance
...
**Aim:** To enable the maximum effect of a field (F max) on a substance, even if the direct application of this effect under the given boundary conditions (e.g. sensitivity, safety) is not permissible or practicable. 
...
- **When to use?** → When contact between two substances is simultaneously beneficial and harmful→ When the harmful effect is of a physical, chemical or mechanical nature→ When insertion of an agent that does not cause significant interference is permitted
...
- **When to use?** → When beneficial effect and harmful side effect occur simultaneously→ When external additives (see 1.2.1) are not desired or permitted→ When it is possible to create an intermediate form (S′ 1,2) from existing resources in the system
...
- **When to use?** → If S1 must not be changed directly → If a field effect (e.g. heat, radiation, pressure, chemical attack) cannot be controlled → If physical shielding is possible and economically viable
...
#### Standard 1.2.4   newtralization of the harmful effect by a second field

Insertion of a field that counteracts the damaging field. If the direct contact between S1 and S2 must be maintained and no new substances are to be added, a second field F₂ is added to the substance-field model, which   newtralizes the harmful effect or converts it into a beneficial effect, while F₁ continues to fulfil its positive function.

**Aim:** If a beneficial effect (F₁) occurs between two substances (S1 and S2), but a harmful effect occurs at the same time, this should be   newtralized by a second field (F₂) or converted into a beneficial effect without interrupting the contact between S1 and S2 and without introducing additional substances.
...
- **When to use?** → When the contact between the substances must be maintained → When no new substances may or should be introduced → When the harmful effect can be eliminated by a physical counterfield

- **Measure:**
   - Add a second field F₂ to the existing model, which specifically   newtralizes or compensates for the negative effect of F₁
...
3. pump noise suppression with counter-sound
   ...
   - F₂: acoustic opposing field (180° phase shift) →   newtralization of the disturbing pump noise

**Notes**

- The effect of F₂ can be   newtralizing or transforming
- F₂ can be e.g:
...
this should be   newtralized by a second field F₂  newtralized by eliminating or compensating for the magnetic properties. 
...
- **When to use?** → When magnetic forces in the system cause unwanted attraction, deflection or repulsion→ When targeted demagnetization is possible→ When complete shielding of the field is not technically or economically feasible
...
- **When to use?** → When a single substance-field model is not strong, efficient or flexible enough→ When the direct coupling of S1 and S2 is problematic or too coarse→ When the effect should be modularized or made more controllable 
- Measure:** 
...
- **When to use?** → When existing SFM works in principle, but is imprecise, slow, not fully effective or not controllable
...
2. hydrostatically mounted component
   - S1: Liquid metal
   - S2: Component
...
The effectiveness of a complete substance-field model can be improved by replacing an uncontrollable or difficult to control field F1 with a well controllable field Fnew, possibly in combination with a new substance Snew. 
with a new substance Snew. 
...
**Aim:** Increasing the effectiveness and controllability of an existing substance-field model (SFM) by replacing or supplementing a field that is difficult to control (F₁) with a more precisely controllable field (Fnew) - possibly in combination with a new substance (Snew). 
in combination with a new substance (Snew).
...
2. inkjet printer with variable droplet output
   - S1: Ink
...
3. Colman™ lantern with structured surface
...
- This solution is closely linked to:
   ...
   - Standard 5.3.1ff - e.g. use of effects such as shape memory, microencapsulation, phase change
...
- **When to use?** → When a field (mechanical, acoustic, electromagnetic, etc.) interacts with a substance → When the system reacts to vibrations, waves or periodic signals → When you want to couple energy or avoid scattering in a targeted manner
...
- **When to use?** → If the substance (S1, S2) cannot be directly replaced by ferroactive substances→ If magnet control is required, but only possible via supplements→ If existing materials are to be made magnetizable, e.g. temporarily
...
- **When to use?** → If a magnetic effect is required, but system components must not be changed→ If the workpiece, tool or component must remain "  newtral" (e.g. non-magnetizable or medically sensitive)→ If the environment is to be made specifically viscous, adhesive or inert
...
5. flexible catamaran
   - Flexible beams instead of rigid connection → variable distance, possibly better ma  newverability
...
- Technically realizable connection types:
   ...
   - Fluidic: hydraulic, p  newmatic coupling
...
2. screwdriver with seal check
   - P  newmatic feedback indicates whether seal is present - without separate measurement           
...
1. Barkhausen effect for crack testing:
...
* Reaction by-products must be considered and   newtralized if necessary
...
1. ionizable gas in lightning rods
   ...
   - The gas then recombines and becomes   newtral again → no more interference → *self-activating protective function.
...
* Analyse and, if necessary,   newtralize by-products, observe energy requirements
...
3. ionization in a gas discharge (e.g. lightning rod)
   ...
   - Afterwards: Recombination to form   newtral gas → *Decomposition and re-synthesis at exactly the right structural level 
